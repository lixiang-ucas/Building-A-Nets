...
train_input_names,train_output_names, val_input_names, val_output_names, test_input_names, test_output_names = prepare_data('./data/mass_buildings/patches')
...
with tf.device('/gpu:0'):
    ...
    network = build_fc_densenet(input, preset_model = 'FC-DenseNet232', num_classes=2, dropout_p=0.4)
...
if is_training:
    ...
    # plt.close(fig)
    np.savetxt('avg_scores_per_epoch.txt',avg_scores_per_epoch)
    np.savetxt('avg_loss_per_epoch.txt',avg_loss_per_epoch)
