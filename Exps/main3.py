...
train_input_names,train_output_names, val_input_names, val_output_names, test_input_names, test_output_names = prepare_data('./data/mass_buildings/patches') #lx modified


print("Setting up training procedure ...")
input = tf.placeholder(tf.float32,shape=[None,None,None,3])
output = tf.placeholder(tf.float32,shape=[None,None,None,2]) #lx modified
network = build_fc_densenet(input, preset_model = 'FC-DenseNet103', num_classes=2)
...
class_names_string = "Building, Unlabelled" #lx modified
class_names_list = ["Building", "Unlabelled"] #lx modified
    ...
    for epoch in range(0, num_epochs):
        ...
        for id in np.random.permutation(len(train_input_names))[:1000]:
            ...
            if input_image_names[id] is None:
                ...
                summary,_,current=sess.run([merged,opt,loss],feed_dict={input:input_image,output:output_image}) #lx modified
                train_writer.add_summary(summary, it) #lx added
                it+=1 #lx added
        ...
        for ind in val_indices:
            ...
            val_writer.add_summary(summary, it2) #lx added
            it2 += 1 #lx added
        ...
        ax2.plot(range(num_epochs), avg_loss_per_epoch)
